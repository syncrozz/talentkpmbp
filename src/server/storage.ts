...
// Default initial dataset (Used ONLY when data/store.json does NOT exist on very first boot)
...
const INITIAL_STUDENTS: Student[] = [
  {
    ...
    preferred_name: 'Aina',
    programme: 'DIA - Diploma in Accountancy',
    semester: 3,
    class: 'DIA3A',
    gender: 'PEREMPUAN',
    phone: '014-5313756',
    email: 'nuraina.batrisyia@bpenawar.kpm.edu.my',
    status: 'ACTIVE',
  ...
  },
  {
    ...
    programme: 'DLM - Diploma in Logistics Management',
    semester: 2,
    class: 'DLM2B',
    gender: 'LELAKI',
    phone: '017-8892143',
    email: 'm.faris@bpenawar.kpm.edu.my',
    status: 'ACTIVE',
  ...
  },
  {
    ...
    programme: 'PRA DIP - Pra Diploma',
    semester: 1,
    class: 'PRADIP1A',
    gender: 'PEREMPUAN',
    phone: '019-3321908',
    email: 'siti.sarah@bpenawar.kpm.edu.my',
    status: 'ACTIVE',
...
const INITIAL_QUESTIONS: OpportunityQuestion[] = [
  {
    question_id: 'q-lb-1',
    opportunity_id: 'opp-001',
...
const INITIAL_APPLICATION_RESPONSES: ApplicationResponse[] = [
...
const INITIAL_APPLICATION_STATUS_HISTORY: ApplicationStatusHistory[] = [
...
const INITIAL_ADMIN_NOTES: AdminNote[] = [
...
const INITIAL_ADMIN_USERS: AdminUser[] = [
  ...
  },
  {
    ...
    email: 'pegawai.hep@kpmbp.edu.my',
    role: AdminRole.ADMIN,
    department: 'Hal Ehwal Pelajar (HEP)',
    status: 'ACTIVE',
    created_at: '2026-01-01T00:00:00Z',
    updated_at: '2026-01-01T00:00:00Z',
  },
  {
    ...
    email: 'panel.kebudayaan@kpmbp.edu.my',
    role: AdminRole.REVIEWER,
    department: 'Panel Uji Bakat & Audisi',
    status: 'ACTIVE',
    created_at: '2026-01-01T00:00:00Z',
    updated_at: '2026-01-01T00:00:00Z',
  },
...
const INITIAL_AUDIT_LOGS: AuditLog[] = [
  {
    ...
    entity_id: 'sys',
    actor_id: 'adm-001',
    actor_name: 'Sistem KPMBP',
    actor_role: 'SUPER_ADMIN',
    timestamp: '2026-08-30T00:00:00Z',
    details: 'Pangkalan data SES 4.4 dimulakan dengan integriti relasional penuh.',
  }
];
...
const INITIAL_NOTIFICATIONS: NotificationItem[] = [
  {
    ...
    title: 'Jemputan Menyertai Public Speaking Team 2026',
    message: 'Pentadbir Unit Bakat KPMBP telah menjemput anda menyertai panggilan terbuka KPMBP Public Speaking Team 2026.',
  ...
  },
  {
    ...
    title: 'Permohonan Baru: Legacy Band 2026',
    message: 'NUR AINA BATRISYIA BINTI ZULHILMI telah menghantar permohonan untuk posisi Lead Guitar.',
    type: 'APPLICATION_SUBMITTED',
    related_entity_id: 'app-001',
    is_read: false,
...
/**
 * Initializes or loads the persistent data store.
 * Strictly adheres to SES 4.4:
 * - If data/store.json exists, load it directly as Single Source of Truth.
 * - If empty array is saved, it stays empty.
 * - No automatic re-seeding on restart when data/store.json exists.
 */
export async function initStorage(): Promise<DataStore> {
  try {
    await fs.mkdir(DATA_DIR, { recursive: true });
    
    try {
      const dataRaw = await fs.readFile(STORE_FILE, 'utf-8');
      const parsed: DataStore = JSON.parse(dataRaw);
      ...
      storeMemory = {
        ...
        opportunityQuestions: parsed.opportunityQuestions || [],
        applications: parsed.applications || [],
        applicationResponses: parsed.applicationResponses || [],
        applicationStatusHistory: parsed.applicationStatusHistory || [],
        adminNotes: parsed.adminNotes || [],
        invitations: parsed.invitations || [],
        participationHistory: parsed.participationHistory || [],
        adminUsers: parsed.adminUsers || INITIAL_ADMIN_USERS,
        auditLogs: parsed.auditLogs || [],
        notifications: parsed.notifications || [],
        pilotFeedbacks: parsed.pilotFeedbacks || [],
      };
      
      console.log('Loaded persistent store from disk (SES 4.4 Data Engine).');
      return storeMemory;
    } catch (readErr: any) {
      // File does not exist -> Create initial store on first install
      const initialStore: DataStore = {
        ...
        opportunityQuestions: INITIAL_QUESTIONS,
        applications: INITIAL_APPLICATIONS,
        applicationResponses: INITIAL_APPLICATION_RESPONSES,
        applicationStatusHistory: INITIAL_APPLICATION_STATUS_HISTORY,
        adminNotes: INITIAL_ADMIN_NOTES,
        invitations: INITIAL_INVITATIONS,
        participationHistory: INITIAL_PARTICIPATION,
      ...
      storeMemory = initialStore;
      console.log('Created initial persistent store in /data/store.json.');
      return storeMemory;
    }
  } catch (err) {
...
/**
 * Saves current in-memory store atomically to disk.
 * Uses temp file + rename to prevent data corruption during write operations.
 */
export async function persistStore(): Promise<void> {
  if (!storeMemory) return;
  const tempFile = `${STORE_FILE}.tmp.${Date.now()}`;
  try {
    const jsonStr = JSON.stringify(storeMemory, null, 2);
    await fs.writeFile(tempFile, jsonStr, 'utf-8');
    await fs.rename(tempFile, STORE_FILE);
  } catch (err) {
    console.error('Failed to write store atomically to disk:', err);
    try {
...
/**
 * Access the authoritative in-memory store.
 */
export function getStore(): DataStore {
  if (!storeMemory) {
    throw new Error('Data store not initialized. Call initStorage() first.');
  }
  return storeMemory;
}

/**
 * Mutates store with automatic persistence.
 */
export async function mutateStore<T>(mutator: (store: DataStore) => T | Promise<T>): Promise<T> {
